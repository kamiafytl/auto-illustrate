...
type RatingFile = Record<string, JsonValue>
...
function scoreFromRatingValue(value: JsonValue): number | null {
  const rawScore = typeof value === 'object' && value !== null && !Array.isArray(value) ? value.score : value
  const score = typeof rawScore === 'number' ? rawScore : typeof rawScore === 'string' && rawScore.trim() ? Number(rawScore) : Number.NaN
  if (!Number.isFinite(score)) return null
  return score
}

function folderFromRatingFileName(fileName: string): string {
  return fileName.replace(/^评分_/, '').replace(/\.json$/i, '')
}

async function readRatingSummary(trackingDir: string): Promise<RatingSummaryItem[]> {
  let entries: { isFile(): boolean; name: string }[]
  try {
    entries = await fs.readdir(trackingDir, { withFileTypes: true })
  } catch (error) {
  ...
  const files = entries
    .filter((entry) => entry.isFile() && /^评分_.*\.json$/i.test(entry.name))
    .map((entry) => entry.name)
    .sort((left, right) => left.localeCompare(right, 'zh-Hans-CN'))

  const summaries = await Promise.all(
    files.map(async (fileName) => {
      const ratings = await readJsonFile<RatingFile>(path.join(trackingDir, fileName))
      ...
      for (const value of values) {
        const score = scoreFromRatingValue(value)
        if (score === null) continue
        const key = Number.isInteger(score) ? String(score) : String(Number(score.toFixed(2)))
      ...
      return {
        folder: folderFromRatingFileName(fileName),
        count: values.length,
        distribution,
        average: ratedCount > 0 ? Number((total / ratedCount).toFixed(2)) : null,
        ratedCount,
      }
    }),
  )
...
async function sendDataImage(res: ServerResponse, imageRoots: string[], rawPath: string | null): Promise<void> {
  ...
  // 依次尝试各图库根：data/images（studio-next 自有扁平图）→ webapp/public/images（原始图库, 只读回退, #6）
...
export function dataReadPlugin(): Plugin {
  return {
    name: 'studio-next-data-read',
    configureServer(server) {
      const dataDir = path.resolve(server.config.root, '..', 'data')
      const trackingDir = path.resolve(server.config.root, '..', 'tracking')
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (!pathname.startsWith('/api/data/') && !pathname.startsWith('/api/rating/')) {
          next()
          return
        }
        ...
        try {
          ...
          if (pathname === '/api/rating/summary') {
            sendJson(res, 200, await readRatingSummary(trackingDir))
            return
          }
